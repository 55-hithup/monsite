    ...
    <div className="testi-card h-full flex flex-col justify-between text-left p-6 sm:p-7 rounded-xl bg-white border border-[#E5E5E5] hover:border-[#0284C7] transition-all duration-300 shadow-sm hover:shadow-md">
      ...
      <div className="testi-person flex items-center gap-3.5 mt-6 pt-4 border-t border-[#F1F5F9]">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="testi-role text-xs text-[#334155] font-['Plus_Jakarta_Sans'] truncate">
            {testi.role}
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 max-w-6xl">
        ...
        {!showForm && (
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-12">
            <a
              href="https://search.google.com/local/writereview?placeid=ChIJDeYFThamsqkR1Uzn7o0t9UY"
              target="_blank"
              rel="noopener noreferrer"
              className="btn-glacier-outline inline-flex items-center gap-2 text-xs"
            >
              <svg viewBox="0 0 24 24" width="16" height="16" className="flex-shrink-0" aria-hidden="true">
                <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
                <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
              </svg>
              <span>{t.leaveGoogleReview}</span>
            </a>
            <button 
              onClick={handleOpenForm} 
              className="btn-glacier-outline inline-flex items-center gap-2 text-xs cursor-pointer"
            >
              <PenSquare size={14} className="text-[#0284C7]" />
